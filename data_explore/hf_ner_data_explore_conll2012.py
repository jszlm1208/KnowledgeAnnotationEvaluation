...
def main():
    ...
    def _extract_info(dataset):
        data = []
        id = 0
        for i, dct in enumerate(dataset):
            sentences = dct['sentences']
            for j, sentence in enumerate(sentences):
                token = sentence['words']
                ner_tags = sentence['named_entities']
                data.append({'id':id, 'tokens':token, 'ner_tags':ner_tags})
                id += 1
        return data
    ...
    id2label = dict(zip(list(range(train.features['sentences'][0]['named_entities'].feature.num_classes)),train.features['sentences'][0]['named_entities'].feature.names))
    label2id = dict(zip(train.features['sentences'][0]['named_entities'].feature.names,list(range(train.features['sentences'][0]['named_entities'].feature.num_classes))))

    # extract relevant info from dataset
    train = _extract_info(train)
    test = _extract_info(test)
    validation = _extract_info(validation)
